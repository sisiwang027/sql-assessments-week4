...
SELECT m.* FROM models AS m, brands AS b where m.brand_id=b.brand_id and b.name='Pontiac';
...
SELECT m.name, b.name, b.headquarters FROM brands AS b, models AS m WHERE b.brand_id=m.brand_id and b.name='Ford' and m.name='Mustang';
...
SELECT COUNT(*) FROM models AS m, brands AS b  WHERE m.brand_id=b.brand_id and b.name='Ford'; -- I have check one brand name just have only on brand id, so I also can use brand id.If I use brand_id='for', join is not nessary. 
SELECT COUNT(*) FROM models AS m, brands AS b  WHERE m.brand_id=b.brand_id and b.name='Ford';
...
select m.year,m.name,m.model_id,a.name from models m left join  awards a on  m.model_id=a.winner_id where m.year >= 1964;
...
SELECT b.name, m.year, m.name FROM brands AS b, models AS m WHERE b.brand_id=m.brand_id ORDER BY b.name;
...
SELECT b.name, b.founded FROM brands AS b LEFT JOIN models AS m ON b.brand_id=m.brand_id where m.model_id IS NULL;
...
SELECT b.brand_id FROM brands AS b LEFT JOIN models AS m ON b.brand_id = m.brand_id GROUP BY b.brand_id HAVING COUNT(m.model_id) > 5;
